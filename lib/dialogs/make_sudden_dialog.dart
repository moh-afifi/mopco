...
import 'package:mopco/dialogs/confirm_sudden_dialog.dart';
import 'package:mopco/dialogs/days_must_dialog.dart';

class MakeSuddenDialog extends StatefulWidget {
  MakeSuddenDialog({this.code, this.docId,this.numOfDeserved});
...
class _MakeSuddenDialogState extends State<MakeSuddenDialog> {
  ...
  void getUsedVacation() async {
    await for (var snapshot in _fireStore.collection("عارضة").snapshots()) {
      for (var message in snapshot.documents) {
        if (widget.code == (message.data['الرقم']).toString()) {
          numOfUsed = (message.data['المستنفذ']).toDouble();
        }
      }
    }
  }
  //--------------------------------------------------------------
  void confirmSuddenDialog(
            ...
            child: FlatButton(
              ...
              onPressed: () {
                if (fromDate == null || toDate == null) {
                  ...
                  Navigator.pop(context);
                  confirmSuddenDialog(fromDate, toDate, numOfDays,
