...
import 'package:mopco/dialogs/confirm_regular_dialog.dart';
import 'package:mopco/dialogs/days_must_dialog.dart';

class MakeRegularDialog extends StatefulWidget {
  MakeRegularDialog({this.code, this.docId,this.numOfDeserved});
...
class _MakeRegularDialogState extends State<MakeRegularDialog> {
  ...
  void getUsedVacation() async {
    await for (var snapshot in _fireStore.collection("سنوي").snapshots()) {
      for (var message in snapshot.documents) {
        if (widget.code == (message.data['الرقم']).toString()) {
            numOfUsed = (message.data['المستنفد']).toDouble();
        }
      }
    }
  }
            ...
            child: FlatButton(
              ...
              onPressed: () {
                if (fromDate == null || toDate == null) {
                  ...
                  Navigator.pop(context);
                  confirmRegularDialog(fromDate, toDate, numOfDays,
