...
import 'package:mopco/dialogs/confirm_hours_dialog.dart';
import 'package:mopco/dialogs/days_must_dialog.dart';

class MakeHourDialog extends StatefulWidget {
  MakeHourDialog({this.code, this.docId,this.numOfDeserved});
  final String code, docId,numOfDeserved;
  @override
  _MakeHourDialogState createState() => _MakeHourDialogState();
}

class _MakeHourDialogState extends State<MakeHourDialog> {
  double numOfDays = 0;
  double numOfUsed = 0;
  ...
  void getUsedVacation() async {
    await for (var snapshot in _fireStore.collection("ساعات").snapshots()) {
      for (var message in snapshot.documents) {
        if (widget.code == (message.data['الرقم']).toString()) {
          numOfUsed = (message.data['المستنفذ من الساعات']).toDouble();
        }
      }
    }
  }
  //--------------------------------------------------------------
  void confirmHoursDialog(
      String from, String to, double numOfDays, String code, double used,String deserved,String docId) {
    showDialog(
      context: context,
      builder: (BuildContext context) => ConfirmHoursDialog(
        from: from,
        to: to,
        numOfDays: numOfDays,
        code: code,
            ...
            child: FlatButton(
              ...
              onPressed: () {
                if (fromDate == null || toDate == null) {
                  ...
                  Navigator.pop(context);
                  confirmHoursDialog(fromDate, toDate, numOfDays,
